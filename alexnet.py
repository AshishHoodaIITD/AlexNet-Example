...
def train():
	...
	scheduler = ls.ReduceLROnPlateau(optimizer,  mode='min', factor=0.1, patience=5)
	...
	while epoch < p:
		...
		for i, data in enumerate(valloader, 0):
			...
			val_input, val_label = Variable(val_input.cuda()), Variable(val_label.cuda())
			val_output = alex_net(val_input)
			loss = criterion(val_output,val_label)
			val_loss+=loss.data[0]
		print('Accuracy of the network on the validation set: %f %% and validation loss: %.3f' % (
	    	acc,val_loss))
